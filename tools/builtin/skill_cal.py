# -*- coding: utf-8 -*-
"""Feishu calendar skill — CLI script for all calendar operations.
...
async def _cli_main() -> None:
    """CLI entry: python3 skill_cal.py <action> --params '<json>'"""
    import argparse
    from config.settings import load_settings
    from platforms.feishu.api import FeishuAPI

    parser = argparse.ArgumentParser(description="Feishu calendar operations")
    parser.add_argument("action", choices=sorted(CAL_ACTIONS))
    ...
    try:
        result = await feishu_cal(args.action, params)
